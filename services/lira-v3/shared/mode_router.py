...
def _data_dir() -> Path:
    return Path(settings.personality_dir).parent
    # personality_dir is "./data/personalities", so parent is "./data/"


PERSONALITY_DIR = _data_dir() / "personalities"
...
def load_snapshot(file: str) -> dict | None:
    ...
    for mode, personality in data.get("personalities", {}).items():
        update_personality(mode, personality)
    return data
